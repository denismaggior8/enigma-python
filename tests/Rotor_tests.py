...
class TestRotor(unittest.TestCase):
    ...
    def test_reset_rotor_position(self):
        rotor = Rotor(alphabet=Alphabets.lookup.get('latin_i18n_26chars_lowercase'),position=25, notch_indexes=[], ring=0, wiring=Alphabets.lookup.get('latin_i18n_26chars_lowercase'))
        rotor.reset_position()
        self.assertEqual(rotor.position, 0, "Rotor position is wrong")
         
    def test_set_rotor_position(self):
        rotor = Rotor(alphabet=Alphabets.lookup.get('latin_i18n_26chars_lowercase'),position=0, notch_indexes=[],wiring=Alphabets.lookup.get('latin_i18n_26chars_lowercase'))
