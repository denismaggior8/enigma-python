...
class Enigma(Observer,Journaled,Clonable):
    ...
    def update(self, observable, *args, **kwargs):
        ...
        for i, rotor in enumerate(self.rotors):
            if rotor is observable:
                observable_index = i
                break
                
        if observable_index != -1:
            # If there is rotor N+1, increment its position by 1
            if observable_index < len(self.rotors) - 1:
                next_rotor = self.rotors[observable_index + 1]
                next_rotor.increment_position()
                logging.debug("Rotor at index {} has been incremented by 1 position".format(observable_index + 1))
            ...
            elif observable_index == len(self.rotors) - 1 and isinstance(self.reflector, RotatingReflector):
                self.reflector.increment_position()
